...
- Whisper ASR (faster-whisper / faster-whisper-xxl) ships anti-hallucination decoding defaults on (`FasterWhisperConfig.AntiHallucination`, default on, since 0.3.6): the engine command gets `--condition_on_previous_text False` (a repetition loop no longer propagates), plus the more permissive `--no_speech_threshold 0.4` and `--vad_threshold 0.35` so speech that co-occurs with background music is not dropped as silence. Each flag is appended only when the user has not already set it in Extra Arguments (de-duplicated; an explicit user value wins), so the behaviour is additive and fully overridable. Turning the toggle off restores the engine's own defaults. (See `config-data-contract.md`.)
- Generated subtitles are re-segmented for readability (default on, `Subtitles.ResegmentSubtitles`): an over-long Whisper cue is split into short, at-most-2-line cues (~42 characters/line) with proportionally redistributed timings, on both the interactive ASR path and batch generation, and the translated text is wrapped to the same shape. Cues that already fit are left untouched; line breaks are stored as `\n`. The default LLM translation prompts were also improved for accuracy (meaning over literal, tone/register preserved, output-only-the-translation); existing hand-edited prompts are preserved.
