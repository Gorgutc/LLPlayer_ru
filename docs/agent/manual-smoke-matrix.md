...
- **F-16 selected-audio correctness owner acceptance — ⚠ PENDING / NOT RUN (planned evening 2026-07-23; not a PASS):** use real media containing video plus at least two clearly distinguishable embedded audio streams (for example, track A says a repeated English marker and track B says a different Russian marker/music bed). Record the container-global FFmpeg stream indexes; select a non-first audio stream whose global index is not its zero-based audio ordinal so the smoke detects global-index/ordinal confusion.
  1. With no `video.ru.srt` or `video.ru.dub.*`, explicitly select the non-first target track, enable dubbing, and run the job. Confirm the generated SRT transcribes that target and the preserved/ducked original bed contains that same target; none of the first track is substituted.
  2. Remove the outputs, set language tags/configured language order so **Auto** resolves the same non-first target, and repeat the ASR+dub checks.
  3. Keep a known `video.ru.srt`, remove only `video.ru.dub.*`, then repeat both manual and Auto selections. Confirm ASR/translation do not run and the original/ducked bed still comes from the selected track.
  4. Queue/resolve a target and then replace or mutate the source before assembly so that resolved global stream index no longer exists. Confirm the job fails, creates no usable dub, and never falls back to the first audio stream. Separately, an override already stale at the initial probe may legitimately fall through to Auto; that initial-resolution behavior must not be mistaken for the post-resolution fail-closed case.
