...
- HC-22 host teardown: start slow translation and definition requests for a fresh sidebar word, then hide the sidebar before either completes. No old popup/result, cancellation snackbar, global dispatcher error, late cache entry, or stuck spinner may appear. Show the sidebar again and confirm a fresh lookup succeeds in the new instance.
...
- HC-22 settings invalidation: during separate slow uncached lookups, change the translation provider, source language, target language, and definition provider one at a time. Each change must close/reset the old popup; its result must not return, enter either cache, or be saved under the new settings.
