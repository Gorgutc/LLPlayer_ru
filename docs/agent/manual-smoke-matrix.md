...
- **T-13e Testing controlled run — PENDING, explicit owner approval required:** only after the hardening PR is green and merged, prove remote `main`, workflow control SHA, dispatch input, run `head_sha`, and selected SHA are the same exact 40-character commit. Dispatch Testing first. Retain the run URL, `.NET 10` and full-preflight evidence, `7z t`, resolved yt-dlp version/size/SHA-256, immutable `testing-<12sha>` tag mapping, draft-prerelease state, and exact `LLPlayer-testing-<12sha>-x64.7z` size/SHA-256. A rerun may `--clobber` only that exact asset while the tag still points directly to the same commit and the Release remains draft; never move/delete the tag or publish the Release.
- **T-13e Stable controlled run — PENDING, explicit owner approval required:** run only after the successful Testing readback above, using the same exact commit. Dispatch the trusted default-branch workflow with that SHA and a tag matching `LLPlayer.csproj` (`v0.3.61` for the current version); do not push the tag manually. Retain the run URL, full-preflight/package/trusted-verification evidence, `7z t`, yt-dlp evidence, direct tag-to-SHA mapping, draft non-prerelease state, and exact `LLPlayer-v0.3.61-x64.7z` size/SHA-256. The workflow must never publish or replace an existing tag/Release; publishing the resulting draft remains a separate owner-only action.
