...
- Batch subtitles is a non-modal singleton dialog opened through `AppActions`. It owns folder selection, scan, queue progress, cancel, and output-folder access without redesigning existing subtitle settings or sidebar behavior. A batch run is decoupled from the player: it keeps running when the main video window is closed (the app minimizes to the system tray instead of quitting — see Main Window Layout), shows overall progress on its own taskbar button + the tray icon, and a video row can be double-clicked to play that file in the main player (restoring it from the tray if hidden). The dialog also exposes background-friendliness toggles (default on) — "Smooth (no ASR/translate overlap)" and "Run ASR on CPU while I work" with an idle-seconds threshold — and shows an "ASR on CPU (you're active)" indicator in its summary.
