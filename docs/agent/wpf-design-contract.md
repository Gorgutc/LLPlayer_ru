...
- The existing per-file audio-track picker in Batch subtitles is the only selected-audio UI for the F-16 correctness slice: its manual/Auto result governs both ASR and the original/ducked bed when dubbing is enabled, including existing-`.ru.srt` render-only jobs. This adds no control, setting, or persisted state; the picker labels and Auto policy remain unchanged.
