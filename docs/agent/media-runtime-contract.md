...
- LLM-like translation has sequential/context modes; do not parallelize away context retention. Batch subtitle translation must keep `KeepContext` sequential within each file. By default completed-file translation overlaps with ASR for the next file (pipelined throughput); a background-friendly "serialize ASR and translation" mode (default on in the app) instead runs each file fully through ASR → translate → save before the next file's ASR, so a GPU ASR engine and a local-LLM translator never both saturate the GPU at once.
- Batch can stay responsive to interactive use without leaving the GPU: an optional app-supplied `IBatchThrottle` lets the batch hold the queue and suspend/resume a running faster-whisper child process (Win32 idle + `NtSuspendProcess`, app layer only) while the user is active. This is additive and must not change ASR/translation results.
