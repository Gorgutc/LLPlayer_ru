...
- LLM-like translation has sequential/context modes; do not parallelize away context retention. Batch subtitle translation must keep `KeepContext` sequential within each file while allowing completed-file translation to overlap with ASR for the next file.
