...
- Both Stable and Testing release workflows must checkout the exact target, use the immutable `setup-dotnet` v5.4.0 action to install the repository's `10.0.x` SDK channel, and complete the canonical full `scripts/codex/verify.ps1` gate before invoking the shared packaging action. This admission gate belongs to the caller so a Testing-selected older commit cannot bypass it merely by carrying an older local composite action; a missing verifier fails before publish/archive.
