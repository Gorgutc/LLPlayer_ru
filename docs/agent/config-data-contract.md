...
- Batch subtitle UI defaults live in `LLPlayer.Config.json`: last folder, recursive scan, overwrite-existing policy, and the background-friendliness knobs `SerializeAsrAndTranslate` (default `true`), `PauseWhenActive` (default `true`), and `ActiveIdleThresholdSeconds` (default `45`). All are additive/absent-defaulting. Batch processing reuses ASR/Translate settings from player config snapshots and must not silently change `TranslateTargetLanguage` or Whisper translate settings in the live player config.
