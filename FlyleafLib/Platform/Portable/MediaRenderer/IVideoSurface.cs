...
/// <para>
/// Frames arrive ready to show: deinterlaced, tone-mapped (HDR), cropped, rotated / mirrored (<see cref="Renderer.Rotation"/>,
/// <see cref="Renderer.HFlip"/>, <see cref="Renderer.VFlip"/> are informational — do not apply them again) and colour
/// filtered. Their pixel size is min(native, <see cref="Renderer.Viewport"/> size) per axis, so it can change with the
/// viewport; always scale the frame into <see cref="Renderer.Viewport"/> whatever its pixel size.
/// </para>
/// <para>
...
public interface IVideoSurface
{
    ...
    /// starting <paramref name="stride"/> bytes after the previous one. The frame is upright and already cropped
    /// (stream/codec + user crop) and at most viewport-sized; the host scales it into <see cref="Renderer.Viewport"/>
    /// (control pixels) and fills the rest of the control with <see cref="VPConfig.BackColor"/>.
    /// </summary>
