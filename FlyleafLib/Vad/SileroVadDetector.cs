...
// Local changes (F-19 slice 2): removed the NAudio dependency — the upstream GetSpeechSegmentList(FileInfo) read a
// WAV with NAudio's AudioFileReader; replaced by GetSpeechSegments(float[] samples) so the detector is fed the
// resampled S16-mono-16k PCM that the ASR pipeline already produces (converted to float upstream). Added
// IDisposable to release the ONNX session. The probability aggregation (CalculateProb / merge) is unchanged.
using System;
...
public class SileroVadDetector : IDisposable
{
    ...
    /// <summary>
    /// F-19 slice 2: run VAD over the resampled S16-mono PCM (as normalized float samples in [-1, 1]) and return
    /// the detected speech segments. Replaces the upstream NAudio WAV reader. The audio must already be at
    /// <c>samplingRate</c> (16 kHz for our ASR pipeline). A trailing partial window (&lt; window size) is ignored.
    /// </summary>
    public List<SileroSpeechSegment> GetSpeechSegments(float[] samples)
    {
        Reset();
        ...
        int pos = 0;
        while (pos + _windowSizeSample <= samples.Length)
        {
            Array.Copy(samples, pos, buffer, 0, _windowSizeSample);
            float speechProb = _model.Call([buffer], _samplingRate)[0];
            speechProbList.Add(speechProb);
            pos += _windowSizeSample;
        }
