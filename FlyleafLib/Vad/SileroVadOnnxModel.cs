...
// Local changes: namespace (VADdotnet -> FlyleafLib.Vad); Dispose() now actually disposes the ONNX session
// (the upstream example only suppressed finalize, leaking the InferenceSession). Inference logic unchanged.
...
public class SileroVadOnnxModel : IDisposable
{
    ...
    public void Dispose()
    {
        // Local change: the upstream example only called SuppressFinalize and never disposed the ONNX
        // InferenceSession — one leaked native session per ASR run. Dispose it here.
        session?.Dispose();
