...
public class GoogleV1TranslateSettings : NotifyPropertyChanged, ITranslateSettings
{
    ...
    {
        List<LanguageRegions> preferences = GoogleV1TranslateService.Regions;
...
public abstract class MicrosoftTranslateSettings : NotifyPropertyChanged, ITranslateSettings
{
    protected MicrosoftTranslateSettings(string defaultEndpoint)
    {
    ...
    public Dictionary<string, string> Regions { get; set; } = new(MicrosoftTranslateServiceBase.DefaultRegions);
    ...
    private ObservableCollection<LanguageRegions> LoadLanguageRegions()
    {
        List<LanguageRegions> preferences = MicrosoftTranslateServiceBase.Regions;
...
public class BingTranslateSettings : MicrosoftTranslateSettings
{
    private const string DefaultEndpointValue = "https://api-edge.cognitive.microsofttranslator.com";

    public BingTranslateSettings() : base(DefaultEndpointValue) { }

    [JsonIgnore]
    public override TranslateServiceType ServiceType => TranslateServiceType.Bing;
}

public class AzureTranslateSettings : MicrosoftTranslateSettings
{
    private const string DefaultEndpointValue = "https://api.cognitive.microsofttranslator.com";

    public AzureTranslateSettings() : base(DefaultEndpointValue) { }

    [JsonIgnore]
    public override TranslateServiceType ServiceType => TranslateServiceType.Azure;

    public string ApiKey { get; set => Set(ref field, value); }

    // optional (null for global)
...
public abstract class OpenAIBaseTranslateSettings : NotifyPropertyChanged, ITranslateSettings
{
    ...
    internal virtual HttpClient GetHttpClient(bool healthCheck = false)
    {
        ...
        HttpClient client = new(handler)
        {
            ...
            Timeout = TimeSpan.FromMilliseconds(healthCheck ? TimeoutHealthMs : TimeoutMs)
        };
        if (!ReuseConnection)
        {
